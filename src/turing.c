...
double call_function(Function *function, double x){
	...
	char rep[50]; // big enough buffer

	sprintf(rep, "%.2f", x);  // format with 2 decimal places
	strcpy(result, str_replace(function->expression, "x", rep));

	return posfix_calculate(result);
}
